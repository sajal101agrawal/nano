...
async function getCandidatesHybridSearch(
  ...
  // Strict hybrid search:
  // - Name match: candidate name contains search term
  // - Full-text match: ALL search words found in profile (using PostgreSQL full-text search)
  // - Skill match: search term matches a skill exactly or closely
  // - Vector match: only if very close to best match (90%+ of max AND 25%+ absolute)
  //
  // NO partial word matching - we require meaningful matches only
  const hybridSql = `
    WITH scored AS (
      SELECT
